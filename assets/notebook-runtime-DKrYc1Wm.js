...
`+n.join(`
`),y=this.blockquote(f);l[l.length-1]=y,s=s.substring(0,s.length-m.raw.length)+y.raw,r=r.substring(0,r.length-m.text.length)+y.text;break}else if(p?.type==="list"){let m=p,f=m.raw+`
`+n.join(`
`),y=this.list(f);l[l.length-1]=y,s=s.substring(0,s.length-p.raw.length)+y.raw,r=r.substring(0,r.length-m.raw.length)+y.raw,n=f.substring(l.at(-1).raw.length).split(`
