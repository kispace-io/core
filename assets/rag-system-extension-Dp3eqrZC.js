...
Use the information from these documents to provide a helpful answer. Pay special attention to numbers, percentages, dates, and specific values mentioned in the snippets. If the documents don't contain relevant information, you can still answer based on your general knowledge.`}formatSearchResults(e){return e.map(t=>({file:t.document.fileName,path:t.document.filePath,relevance:t.relevance.toFixed(2),language:t.document.metadata.language,size:t.document.metadata.size,snippets:t.matchedSnippets,preview:this.snippetExtractor.extractSimpleSnippet(t.document.content,200)+"..."}))}formatCommandResults(e){return e.map(t=>({file:t.document.fileName,path:t.document.filePath,relevance:t.relevance,snippets:t.matchedSnippets}))}}const A=C("RAGService"),T=new te,it=new at(T);class st{async searchDocuments(e,t={}){const a=Math.min(t.limit||Q.DEFAULT_LIMIT,Q.MAX_LIMIT),n=await X(t.workspacePath);if(!n)return A.warn("No workspace connected, cannot search documents"),[];const{workspacePath:i}=n;if(t.filePath){const s=await v.getDocumentByPath(i,t.filePath);if(s){const c=B(e),o=c.length>0?T.extractSnippets(s.content,c,3):[T.extractSimpleSnippet(s.content,F.PREVIEW)];return[{document:s,relevance:100,matchedSnippets:o}]}return A.warn(`Document not found: ${t.filePath}`),[]}if(t.fileName){const s=await v.listDocuments(i),c=t.fileName.toLowerCase(),o=s.filter(l=>{const d=l.fileName.toLowerCase();return d===c||d.includes(c)});if(o.length>0){const l=[],d=B(e);for(const m of o.slice(0,a)){const g=d.length>0?T.extractSnippets(m.content,d,3):[T.extractSimpleSnippet(m.content,F.PREVIEW)];l.push({document:m,relevance:ue.calculateFileNameRelevance(m,t.fileName),matchedSnippets:g})}return l}return A.warn(`No documents found with name: ${t.fileName}`),[]}if(!e||!e.trim())return A.warn("No query provided and no filePath/fileName specified"),[];try{const s=await v.searchSimilar(e,{limit:a*2,workspacePath:i,fileType:t.fileType}),c=[];for(const{document:o,similarity:l,chunkStartOffset:d,chunkEndOffset:m}of s){if(!this.matchesContextScope(o,t.documentSearchScope))continue;const g=l*100;if(t.minRelevance&&g<t.minRelevance)continue;const p=fe(e),u=B(e);let w;if(d!==void 0&&m!==void 0){const f=o.content.substring(Math.max(0,d),Math.min(o.content.length,m));f.trim().length>0?w=[f.trim()]:w=T.extractSnippets(o.content,u,15)}else w=T.extractSnippets(o.content,u,15);if(w.length===0&&p.length>0)if(d!==void 0&&m!==void 0){const f=o.content.substring(Math.max(0,d),Math.min(o.content.length,m));f.trim().length>0?w=[f.trim()]:w=T.extractSnippets(o.content,[p],10)}else w=T.extractSnippets(o.content,[p],10);if(c.push({document:o,relevance:g,matchedSnippets:w}),c.length>=a)break}return c.length===0?(A.debug("Vector search returned no results, falling back to text search"),this.fallbackTextSearch(e,t)):c}catch(s){return A.warn(`Vector search failed, falling back to text search: ${s}`),A.debug(`Vector search error details: ${s}`),this.fallbackTextSearch(e,t)}}async fallbackTextSearch(e,t={}){const a=Math.min(t.limit||Q.DEFAULT_LIMIT,Q.MAX_LIMIT),n=await X(t.workspacePath);if(!n)return A.warn("No workspace connected, cannot perform text search"),[];const{workspacePath:i}=n,s=await v.listDocuments(i),c=B(e),o=[];for(const l of s){if(t.fileType&&l.fileType!==t.fileType||!this.matchesContextScope(l,t.documentSearchScope))continue;const d=ue.calculateRelevance(l,e);if(t.minRelevance&&d<t.minRelevance)continue;const m=T.extractSnippets(l.content,c,3);o.push({document:l,relevance:d,matchedSnippets:m})}return o.sort((l,d)=>d.relevance-l.relevance),o.slice(0,a)}formatRAGContext(e){return it.formatRAGContext(e)}matchesContextScope(e,t){if(!t)return!0;if(t.includePaths&&t.includePaths.length>0&&!t.includePaths.some(n=>n.includes("*")||n.includes("?")?new RegExp("^"+n.replace(/\*/g,".*").replace(/\?/g,".")+"$").test(e.filePath):e.filePath.startsWith(n)||e.filePath===n)||t.excludePaths&&t.excludePaths.length>0&&t.excludePaths.some(n=>n.includes("*")||n.includes("?")?new RegExp("^"+n.replace(/\*/g,".*").replace(/\?/g,".")+"$").test(e.filePath):e.filePath.startsWith(n)||e.filePath===n)||t.pathPattern&&!(t.pathPattern instanceof RegExp?t.pathPattern:new RegExp(t.pathPattern)).test(e.filePath))return!1;if(t.tags&&t.tags.length>0){const a=e.metadata.tags||[];if(!t.tags.every(i=>a.includes(i)))return!1}return!(t.metadataFilter&&!t.metadataFilter(e))}}const rt=new st;async function ot(r,e={}){const t=await X(e.workspacePath);return t?rt.searchDocuments(r,{...e,workspacePath:t.workspacePath}):(A.warn("No workspace connected, cannot search documents"),[])}var ct=Object.defineProperty,lt=Object.getOwnPropertyDescriptor,D=(r,e,t,a)=>{for(var n=a>1?void 0:a?lt(e,t):e,i=r.length-1,s;i>=0;i--)(s=r[i])&&(n=(a?s(e,t,n):s(n))||n);return a&&n&&ct(e,t,n),n};const U=C("RAGSystemManager"),me=new te;let b=class extends Te{constructor(){super(...arguments),this.documents=[],this.stats={totalDocuments:0,byWorkspace:{}},this.loading=!1,this.selectedDocument=null,this.searchQuery="",this.filterWorkspace=null,this.filterByActiveWorkspace=!0,this.filteredDocuments=[],this.searchResults=new Map,this.reindexing=!1,this.treeRef=ye(),this.searchInputValue=""}async doInitUI(){try{await v.initialize(),await Promise.all([this.loadDocuments(),this.loadStats()])}catch(r){U.error(`Failed to initialize document index manager: ${r}`),y(`Failed to initialize: ${r}`)}}async loadDocuments(){this.loading=!0,this.requestUpdate();try{let r;this.filterByActiveWorkspace&&(r=(await X())?.workspacePath),this.documents=await v.listDocuments(r),await this.updateFilteredDocuments()}catch(r){U.error(`Failed to load documents: ${r}`),y(`Failed to load documents: ${r}`)}finally{this.loading=!1}}async updateFilteredDocuments(){this.filteredDocuments=await this.getFilteredDocuments(),this.requestUpdate()}async loadStats(){try{this.stats=await v.getStats(),this.requestUpdate()}catch(r){U.error(`Failed to load stats: ${r}`)}}handleTreeSelection(r){let e=r.detail?.selection||[];if(e.length===0&&this.treeRef.value&&(e=this.treeRef.value.selectedItems||[]),e.length>0){const t=e[0];t?.model?this.selectedDocument=t.model:this.selectedDocument=null}else this.selectedDocument=null}async getFilteredDocuments(){if(!this.documents||this.documents.length===0)return[];if(this.searchQuery.trim())try{const e=await ot(this.searchQuery,{limit:50,workspacePath:this.filterWorkspace||void 0});this.searchResults.clear();const t=new Map,a=new Map;for(const i of e){const s=i.document.id;t.set(s,i.document);const c=a.get(s);if(!c)a.set(s,{document:i.document,relevance:i.relevance,matchedSnippets:[...i.matchedSnippets]});else{i.relevance>c.relevance&&(c.relevance=i.relevance);const o=new Set(c.matchedSnippets);for(const l of i.matchedSnippets)o.has(l)||(c.matchedSnippets.push(l),o.add(l))}}for(const[i,s]of a)this.searchResults.set(i,s);const n=Array.from(t.values());return this.filterWorkspace?n.filter(i=>i.workspacePath===this.filterWorkspace):n}catch(e){return U.debug(`RAG search failed in document manager: ${e}`),this.searchResults.clear(),[]}else this.searchResults.clear();let r=[...this.documents];return this.filterWorkspace&&(r=r.filter(e=>e.workspacePath===this.filterWorkspace)),r}highlightMatches(r,e){if(!e||!e.trim())return x`${r}`;const t=e.toLowerCase(),a=r.toLowerCase(),n=[];let i=0,s=a.indexOf(t,i);for(;s!==-1;)s>i&&n.push(r.substring(i,s)),n.push(x`<mark class="search-match">${r.substring(s,s+e.length)}</mark>`),i=s+e.length,s=a.indexOf(t,i);return i<r.length&&n.push(r.substring(i)),x`${n}`}getContentPreview(r){const e=this.searchResults.get(r.id);if(e&&e.matchedSnippets.length>0)return x`
            ...
            `;if(this.searchQuery&&this.searchQuery.trim()){const a=me.extractContextSnippets(r.content,this.searchQuery,F.CONTEXT);if(a.length>0)return x`
                    <table class="snippets-table">
                        <thead>
                            <tr>
                                <th class="snippet-number-col">#</th>
        ...
        `}async deleteDocument(r){try{await v.deleteDocument(r.id),k(`Deleted: ${r.fileName}`),await this.loadDocuments(),await this.loadStats(),this.selectedDocument?.id===r.id&&(this.selectedDocument=null)}catch(e){y(`Failed to delete document: ${e}`)}}async reindexDocument(r){try{const e=await X();if(!e){y("No workspace connected");return}const t=await e.workspace.getResource(r.filePath);if(!t){y(`File not found: ${r.filePath}`);return}if(!(t instanceof E)){y(`Resource is not a file: ${r.filePath}`);return}const a=t;await R.runAsync("Reindexing document",async n=>{n.message=`Reindexing ${r.fileName}...`,await v.reindexDocument(a),n.progress=100}),k(`Reindexed: ${r.fileName}`),await this.loadDocuments(),this.selectedDocument?.id===r.id&&(this.selectedDocument=await v.getDocument(r.id))}catch(e){y(`Failed to reindex document: ${e}`)}}async reindexAllDocuments(){if(this.reindexing)return;const r=await v.getStats();if(r.totalDocuments===0){k("No documents to reindex");return}this.reindexing=!0,this.requestUpdate();try{const e=await R.runAsync("Reindexing all documents",async t=>{t.message="Starting reindexing...";const a=r.totalDocuments,n=await v.reindexAllDocuments(),i=n.succeeded+n.failed;return t.progress=a>0?i/a*100:100,t.message=`Reindexed ${n.succeeded}/${a} documents${n.failed>0?` (${n.failed} failed)`:""}`,n});await this.loadDocuments(),await this.loadStats(),k(`Reindexing completed: ${e.succeeded} succeeded, ${e.failed} failed`)}catch(e){U.error(`Failed to reindex all documents: ${e}`),y(`Failed to reindex all documents: ${e}`)}finally{this.reindexing=!1,this.requestUpdate()}}formatFileSize(r){return r<1024?`${r} B`:r<1024*1024?`${(r/1024).toFixed(1)} LyraB`:`${(r/(1024*1024)).toFixed(1)} MB`}formatDate(r){return new Date(r).toLocaleString()}getFileIcon(r){return ee.getFileIcon(r)}renderToolbar(){const r=Object.keys(this.stats?.byWorkspace||{});return x`
            <wa-input
                type="search"
    ...
    `;D([xe({attribute:!1})],b.prototype,"input",2);D([P()],b.prototype,"documents",2);D([P()],b.prototype,"stats",2);D([P()],b.prototype,"loading",2);D([P()],b.prototype,"selectedDocument",2);D([P()],b.prototype,"searchQuery",2);D([P()],b.prototype,"filterWorkspace",2);D([P()],b.prototype,"filterByActiveWorkspace",2);D([P()],b.prototype,"filteredDocuments",2);D([P()],b.prototype,"searchResults",2);D([P()],b.prototype,"reindexing",2);b=D([$e("lyra-rag-system-manager")],b);const M=C("RAGSystemExtension");function dt(r){v.initialize().catch(e=>{M.error(`Failed to initialize document index service: ${e}`)}),O({command:{id:"rag-system.index-file",name:"Index Document",description:"Index the currently selected file for search and retrieval",parameters:[{name:"includeContent",description:"Whether to include full content in index (default: true)",required:!1}]},handler:{canExecute:e=>_.get()instanceof E,execute:async e=>{const t=_.get();if(!(t instanceof E)){y("Please select a file to index");return}const a=e.params?.includeContent!==!1;await R.runAsync("Indexing document",async n=>{n.message=`Indexing ${t.getName()}...`;try{const i=await v.indexDocument(t,{includeContent:a});n.progress=100,k(`Document indexed: ${i.fileName}`)}catch(i){throw y(`Failed to index document: ${i}`),i}})}}}),O({command:{id:"rag-system.index-workspace",name:"Index Workspace",description:"Index all indexable files in the current workspace",parameters:[{name:"includeContent",description:"Whether to include full content in index (default: true)",required:!1},{name:"maxFileSize",description:"Maximum file size in bytes to index (default: 5MB)",required:!1}]},handler:{canExecute:e=>!0,execute:async e=>{const t=await V.getWorkspace();if(!t){y("No workspace selected");return}const a=e.params?.includeContent!==!1,n=e.params?.maxFileSize?parseInt(e.params.maxFileSize):void 0;await R.runAsync("Indexing workspace",async i=>{i.message="Collecting files...",i.progress=0;try{const s=await v.indexWorkspace(t,{includeContent:a,maxFileSize:n});i.progress=100,s.failed>0?y(`Indexing complete: ${s.indexed} indexed, ${s.failed} failed. Check console for details.`):k(`Workspace indexed: ${s.indexed} documents`)}catch(s){throw y(`Failed to index workspace: ${s}`),s}})}}}),O({command:{id:"rag-system.list-documents",name:"List Indexed Documents",description:"List all indexed documents in the current workspace",parameters:[]},handler:{canExecute:e=>!0,execute:async e=>{const a=(await V.getWorkspace())?.getName();await R.runAsync("Loading indexed documents",async n=>{try{const i=await v.listDocuments(a);n.progress=100,i.length===0?k("No documents indexed in this workspace"):(M.info(`Found ${i.length} indexed documents`),k(`Found ${i.length} indexed documents (check console for details)`))}catch(i){throw y(`Failed to list documents: ${i}`),i}})}}}),O({command:{id:"rag-system.delete-document",name:"Delete Document from Index",description:"Remove the selected file from the document index",parameters:[]},handler:{canExecute:e=>_.get()instanceof E,execute:async e=>{const t=_.get();if(!(t instanceof E)){y("Please select a file to remove from index");return}const n=t.getWorkspace().getName(),i=t.getWorkspacePath();await R.runAsync("Deleting document from index",async s=>{try{const c=await v.deleteDocumentByPath(n,i);s.progress=100,c?k(`Document removed from index: ${t.getName()}`):k(`Document not found in index: ${t.getName()}`)}catch(c){throw y(`Failed to delete document from index: ${c}`),c}})}}}),O({command:{id:"rag-system.clear-workspace",name:"Clear Workspace Index",description:"Remove all indexed documents from the current workspace",parameters:[]},handler:{canExecute:e=>!0,execute:async e=>{const t=await V.getWorkspace();if(!t){y("No workspace selected");return}const a=t.getName();await R.runAsync("Clearing workspace index",async n=>{try{const i=await v.deleteWorkspace(a);n.progress=100,k(`Removed ${i} documents from index`)}catch(i){throw y(`Failed to clear workspace index: ${i}`),i}})}}}),O({command:{id:"rag-system.get-stats",name:"Document Index Statistics",description:"Get statistics about the document index",parameters:[]},handler:{canExecute:e=>!0,execute:async e=>{await R.runAsync("Loading statistics",async t=>{try{const a=await v.getStats();t.progress=100,M.info(`Document index statistics: ${JSON.stringify(a)}`),k(`Index statistics: ${a.totalDocuments} total documents. Check console for details.`)}catch(a){throw y(`Failed to get statistics: ${a}`),a}})}}}),O({command:{id:"rag-system.reindex-file",name:"Reindex Document",description:"Reindex the selected file (useful after file changes)",parameters:[]},handler:{canExecute:e=>_.get()instanceof E,execute:async e=>{const t=_.get();if(!(t instanceof E)){y("Please select a file to reindex");return}await R.runAsync("Reindexing document",async a=>{a.message=`Reindexing ${t.getName()}...`;try{const n=await v.reindexDocument(t);a.progress=100,k(`Document reindexed: ${n.fileName}`)}catch(n){throw y(`Failed to reindex document: ${n}`),n}})}}}),M.info("RAG system extension loaded"),ee.registerEditorInputHandler({editorId:"system.rag-system-manager",label:"RAG System Manager",ranking:1e3,canHandle:e=>e.key===".system.rag-system",handle:async e=>(e.component=()=>x`
                <lyra-rag-system-manager .input=${e}></lyra-rag-system-manager>
            `,e)}),O({command:{id:"open-rag-system-manager",name:"Open RAG System Manager",description:"Opens the RAG system manager to view and manage indexed documents",parameters:[]},handler:{execute:e=>{const t={title:"RAG System Manager",data:{},key:".system.rag-system",icon:"database",state:{}};ee.loadEditor(t).catch(a=>{M.error(`Failed to open document index manager: ${a}`)})}},contribution:{target:Pe,icon:"database",label:"RAG System"}}),Ie.registerContribution("contextmenu:filebrowser",{name:"contextmenu.filebrowser.indexDocument",command:"rag-system.index-file",icon:"database",label:"Index Document",disabled:()=>!(_.get()instanceof E)}),z(()=>import("./rag-integration-D2ORMkO8.js"),__vite__mapDeps([2,3,0,1,4,5,6])).then(e=>{e.integrateRAGWithAI(),M.info("RAG integration enabled")}).catch(e=>{M.warn(`Failed to load RAG integration: ${e}`)})}const gt=Object.freeze(Object.defineProperty({__proto__:null,default:dt},Symbol.toStringTag,{value:"Module"}));export{at as R,Q as S,te as a,gt as b,v as d,X as g,rt as r,ot as s};
