...
|-------|----------|-------------|
| `id` | Yes | Unique id (e.g. `@eclipse-docks/extension-command-palette`, typically the package name). |
...
Each extension typically lives in its own package under `packages/extension-*`, depends on `@eclipse-docks/core`, and has an entry file that registers the extension and a loader that imports the module that does the real registration.
